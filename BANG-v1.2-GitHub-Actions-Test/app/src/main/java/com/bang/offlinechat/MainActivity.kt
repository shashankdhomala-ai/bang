...
class MainActivity : Activity() {
    ...
    inner class MeshBridge {
        ...
        @JavascriptInterface fun sendTo(text: String, targetName: String) { mesh.send(text, targetName) }
        ...
        @JavascriptInterface fun startAudioServer() { runOnUiThread { requestAudioPermissions { startCallService(); audio.startServer(onConnected = { sendCallEvent("audio_connected", "", "") }) } } }
    ...
    private fun requestMeshPermissionsAndStart() {
        val missing = mutableListOf<String>()
        if (Build.VERSION.SDK_INT >= 31) listOf(Manifest.permission.BLUETOOTH_SCAN, Manifest.permission.BLUETOOTH_ADVERTISE, Manifest.permission.BLUETOOTH_CONNECT).forEach { if (checkSelfPermission(it) != PackageManager.PERMISSION_GRANTED) missing += it }

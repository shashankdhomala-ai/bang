...
class MainActivity : Activity() {
    ...
    inner class MeshBridge {
        ...
        @JavascriptInterface fun connect(address: String) { runOnUiThread { mesh.peerList().firstOrNull { it.address == address }?.let { mesh.connect(it) } } }
    }
    ...
    private fun requestAudioPermissions(afterGranted: () -> Unit) {
        ...
        if (missing.isNotEmpty()) { pendingAudioAction = afterGranted; requestPermissions(missing.toTypedArray(), audioPermissionRequest); return }
        afterGranted()
    }
    private fun requestAudioPermissionsAndStartWifi() { requestAudioPermissions { wifiCalls.start() } }
    private fun startCallService() { val intent = Intent(this, BangCallService::class.java); if (Build.VERSION.SDK_INT >= 26) startForegroundService(intent) else startService(intent) }
    private fun stopCallService() { stopService(Intent(this, BangCallService::class.java)) }

    private fun requestMeshPermissionsAndStart() {
        val missing = mutableListOf<String>()
        if (Build.VERSION.SDK_INT >= 31) {
            listOf(Manifest.permission.BLUETOOTH_SCAN, Manifest.permission.BLUETOOTH_ADVERTISE, Manifest.permission.BLUETOOTH_CONNECT).forEach { if (checkSelfPermission(it) != PackageManager.PERMISSION_GRANTED) missing += it }
        } else if (Build.VERSION.SDK_INT >= 23 && checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) missing += Manifest.permission.ACCESS_FINE_LOCATION
    ...
    private fun enableBluetoothIfNeeded() {
        ...
        if (adapter != null && !adapter.isEnabled) { startActivity(Intent(BluetoothAdapter.ACTION_REQUEST_ENABLE)); return }
        mesh.start()
    }
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        ...
        if (requestCode == audioPermissionRequest && grantResults.isNotEmpty() && grantResults.all { it == PackageManager.PERMISSION_GRANTED }) {
            val action = pendingAudioAction; pendingAudioAction = null; action?.invoke() ?: wifiCalls.start()
        } else if (requestCode == audioPermissionRequest) {
            pendingAudioAction = null; sendMeshEvent("error", "Microphone/Nearby permission was not granted.")
    ...
    private fun sendCallEvent(event: String, from: String, target: String) { runOnUiThread { val js = "window.bangCallEvent && window.bangCallEvent(${JSONObject.quote(event)},${JSONObject.quote(from)},${JSONObject.quote(target)});"; if (::web.isInitialized) web.evaluateJavascript(js, null) } }
    private fun sendMeshMessage(text: String, peerName: String) { runOnUiThread { val js = "window.bangMeshMessage && window.bangMeshMessage(${JSONObject.quote(text)},${JSONObject.quote(peerName)});"; if (::web.isInitialized) web.evaluateJavascript(js, null) } }

    override fun onBackPressed() { if (web.canGoBack()) web.goBack() else super.onBackPressed() }
    override fun onDestroy() { audio.stop(); stopCallService(); relay.stop(); wifiCalls.stop(); mesh.stop(); if (::web.isInitialized) { web.stopLoading(); web.destroy() }; super.onDestroy() }
}

...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        wifiCalls = WifiDirectCallManager(
            this,
            object : WifiDirectCallManager.Listener {

                override fun onStatus(text: String) {
                    sendMeshEvent("wifi_status", text)
                }

                override fun onGroupOwner(host: String) {
                    sendMeshEvent("wifi_host", host)
                }

                override fun onPeer(name: String, address: String) {
                    sendMeshPeer(
                        MeshManager.Peer(address, name)
                    )
                }
        ...
        mesh = MeshManager(
            this,
            object : MeshManager.Listener {

                override fun onStatus(text: String) {
                    sendMeshEvent("status", text)
                }

                override fun onPeerFound(peer: MeshManager.Peer) {
                    sendMeshPeer(peer)
                }
            ...
            override fun onPageFinished(
                ...
                if (api.isNotBlank()) {
                    ...
                    view.evaluateJavascript(
                        "localStorage.setItem('bang_api','$escaped');",
                        null
                    )
                }
            }
    ...
    inner class MeshBridge {
        ...
        fun startWifiCalls() {
            runOnUiThread {
                requestAudioPermissionsAndStartWifi()
            }
        }
    ...
    override fun onRequestPermissionsResult(
        ...
        if (
            requestCode == audioPermissionRequest &&
            ...
            sendMeshEvent(
                "error",
                "Microphone/Nearby permission was not granted."
            )

        } else if (
            requestCode == meshPermissionRequest &&
            grantResults.isNotEmpty() &&
            grantResults.all {
                it == PackageManager.PERMISSION_GRANTED
            }
    ...
    private fun sendMeshPeer(
        ...
        runOnUiThread {
            ...
            val js =
                "window.bangMeshPeer && " +
                "window.bangMeshPeer(" +
                "${JSONObject.quote(peer.name)}," +
                "${JSONObject.quote(peer.address)}" +
                ");"
    ...
    private fun sendMeshMessage(
        ...
        runOnUiThread {
            ...
            val js =
                "window.bangMeshMessage && " +
                "window.bangMeshMessage(" +
                "${JSONObject.quote(text)}," +
                "${JSONObject.quote(peerName)}" +
                ");"

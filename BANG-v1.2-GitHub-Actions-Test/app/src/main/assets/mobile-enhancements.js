(function () {
  ...
  style.textContent = `
    ...
    .bang-mobile-toast { position:fixed; left:50%; bottom:94px; z-index:10000; max-width:calc(100% - 30px); padding:13px 17px; border-radius:13px; color:#fff; background:#202943; box-shadow:0 12px 35px rgba(0,0,0,.35); transform:translate(-50%,20px); opacity:0; pointer-events:none; transition:.25s ease; text-align:center; font-size:14px; }
  ...
  function addMobileNav() {
    ...
    items.forEach(([view, icon, label]) => {
      ...
      b.type = 'button'; b.dataset.view = view;
  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (!active) return;
    if (['u','p','username','password','n','displayName'].includes(active.id) && typeof window.login === 'function') {
      event.preventDefault(); window.login();
    }
  });
  ...
  } else window.toast = showMobileToast;
